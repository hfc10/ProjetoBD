...
public class PedidoDAO extends ConnectionDAO {
    ...
    public boolean insertPedido(Pedido pedido) {
        ...
        boolean sucesso;
        String sql = "INSERT INTO pedido (mesaIdMesa, clienteIdCliente, data) VALUES (?, ?, ?)";
        try {
            pst = con.prepareStatement(sql);
            pst.setInt(1, pedido.getMesaIdMesa());
            pst.setInt(2, pedido.getClienteIdCliente());
            pst.setString(3, pedido.getData().toString());
            pst.execute();
            sucesso = true;
    ...
    public boolean updatePedido(int id, Pedido pedido) {
        ...
        boolean sucesso;
        String sql = "UPDATE pedido SET mesaIdMesa = ?, clienteIdCliente = ?, Data = ?, WHERE idPedido = ?";
        try {
            pst = con.prepareStatement(sql);
            pst.setInt(1, pedido.getMesaIdMesa());
            pst.setInt(2, pedido.getClienteIdCliente());
    ...
    public ArrayList<Pedido> selectPedido() {
        ...
        try {
            ...
            while (rs.next()) {
                Pedido pedidoaux = new Pedido(rs.getInt("IdPedido"), rs.getInt("mesaIdMesa"), rs.getInt("clienteIdCliente"), rs.getDate("data"));
